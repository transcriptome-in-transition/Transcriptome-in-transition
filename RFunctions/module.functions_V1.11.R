...
# V1.11 14-08-2015  Erik  Add: Function for performing ANOVA analyses on module eigengenes using Tina's design: 'run.aov.mod'
...
### Defining a Subfunction for running ANOVAs on module eigengenes with treatments defined in Tina's experiment:
run.aov.mod <- function(dat,net,phen,method="fdr",pcut=0.001,includeCALL=TRUE){
  TM1 <- phen$Larval.food.level
  TM2 <- phen$Adult.food.level
  MEandPhen <- cbind(get.module.eg(dat,net),TM1,TM2)
  # Finding the columns with module eigen-genes:
  mods <- grep("ME",colnames(MEandPhen))
  # Looping over these columns and performing an ANOVA:
  AOV <- t(sapply(MEandPhen[,mods], function(mod){
    summary(aov(mod ~ factor(TM1) * factor(TM2),data=MEandPhen))[[1]]$"Pr(>F)"[1:3]
  }))
  colnames(AOV) <- c("Larval","Adult","LarvalxAdult")
  # Adjust for all:
  AOV <- cbind(AOV,matrix(p.adjust(unlist(AOV),method=method),nrow=nrow(AOV),byrow=FALSE))
  colnames(AOV)[4:6] <- paste0(method,"_",colnames(AOV)[1:3])
  AOV <- signif(AOV,digits=3)
  # CALL:
  if(includeCALL){
    AOV <- cbind(AOV,AOV[,4:6]<=pcut)
    colnames(AOV)[7:9] <- paste0("sign_",colnames(AOV)[1:3])
  }
  return(AOV)
}
