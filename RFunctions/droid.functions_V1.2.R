...
# V1.1  03-09-2015  Erik  Added functionality for miRNAs: 'get.DroID.mir', 'renew.DroID.mir', 'load.DroID.mir'
#                         Deprecated: 'mod.tf_droid.enr' to also work on miRNA-Gene annotations.
#                         Added 'mod.droid.enr' taking over 'mod.tf_droid.enr' at default settings, but type='MIR' will now
#                           perform the same enrichment analysis for miRNA-Gene annotations.
# V1.2  04-09-2015  Erik  Added 'plot.enr.volcano' to inspect the enrichment results of TF/MIR in modules
...
## Defining a SubFunction for downloading TF-GENE links from DroID:
get.DroID.mir <- function(pathIN="http://www.droidb.org/data/DroID_v2014_10/rna_gene.txt"){
  # 1] Downloading data:
  cat("| Downloading data ...")
  dat <- read.delim(pathIN,stringsAsFactors = FALSE)
  cat("OK!\n")
  ...
  dat <- merge(dat,FB2Affy,by.x="FLY_TARGET_GENE",by.y="fbID",all.x=TRUE)
  colnames(dat)[which(colnames(dat)=="affyID")] <- "affyID_GENE"
  dat <- merge(dat,FB2Affy,by.x="FLY_RNA_GENE",by.y="fbID",all.x=TRUE)
  colnames(dat)[which(colnames(dat)=="affyID")] <- "affyID_RNA"
  cat("OK!\n")
  return(dat)
}

## Defining a SubFunction for storing a fresh copy of RNA-Gene links (miR) from DroID:
renew.DroID.mir <- function(folderOUT,pathIN="http://www.droidb.org/data/DroID_v2014_10/rna_gene.txt"){
  cat("=== Renewing DroID miRNA-Gene ===\n")
  # 1) Download & map to affyIDs:
  MIR <- get.DroID.mir(pathIN=pathIN)
  # 2) Store:
  tag <- toupper(gsub("DroID_","",grep("DroID",unlist(strsplit(pathIN,"/")),value=TRUE)))
  pathOUT <- make.path(folderOUT=folderOUT,dataID="mir",tag=tag)
  save(MIR,file=pathOUT)
  cat(paste0("| DroID miRNA-GENE mappings were stored under: '",basename(pathOUT),"'\n"))
  cat("DONE!\n")
}

## Defining a SubFunction for loading the latest version of TF-GENE:
load.DroID.mir <- function(folderIN){
  pathIN <- get.path(folderIN,dataID="mir",db="DroID",latest=TRUE)
  if(length(pathIN)==0){
    stop("Could not find a local copy in the designated path ...")
  }
  cat(paste0("| Loading prepared DroID miRNA-GENE Mappings: ",basename(pathIN)," ..."))
  MIR <- get(load(pathIN))
  cat(" OK! \n")
  return(MIR)
}
...
## Defining a SubFunction for computing TF/mir enrichments for modules:
mod.droid.enr <- function(dat,net,folderIN=NULL,type="TF"){
  allowed.types <- c("TF","MIR")
  if((!any(type %in% allowed.types)) | length(type)>1){
    stop(paste0("Supply for 'type' one of: '",paste(allowed.types,collapse="', '"),"'"))
  }
  ## Preparing database:
  if(type=="TF"){
    cat("=== Performing TF Enrichments [DroID] === \n")
    if(is.null(folderIN)){
      groupings <- get.DroID.TF()
    } else {
      groupings <- load.DroID.TF(folderIN)
    }
    colnames(groupings) <- gsub("TF","GROUP",colnames(groupings))
  } else {
    cat("=== Performing MIR Enrichments [DroID] === \n")
    if(is.null(folderIN)){
      groupings <- get.DroID.mir()
    } else {
      groupings <- load.DroID.mir(folderIN)
    }
    colnames(groupings) <- gsub("RNA","GROUP",colnames(groupings))
  }
  # Subselect and make sure that these entries are unique:
  groupings <- base::unique(groupings[,c("FLY_TARGET_GENE","FLY_GROUP_GENE","GENE_SYMBOL","GROUP_SYMBOL","affyID_GENE")])
  # Extracting GROUP-GENE groupings:
  groupings <- split(groupings$FLY_TARGET_GENE,as.factor(groupings$GROUP_SYMBOL))
  ...
  cat(paste0("| Pruning modules for genes in DroID-",type," ... "))
  L <- ontologyprune(geneList,groupings)
  cat("OK!\n")
  # Tests:
  ...
  res <- list(p=p,OR=OR,low=low,high=high,Nmod_test=L$N1_end,Ngroup_test=L$N2_end,x11=X11,x12=X12,x21=X21,x22=X22)
  return(res)
}

# Plot volcano plot of enrichments:
plot.enr.volcano <- function(res,groupnames=NULL,pmax=12,ORmax=6,main=NULL){
  layout(rbind(1,2),widths=c(1,1),heights=c(9,1))
  p <- -log10(res$p)
  p[which(p>pmax)] <- pmax
  OR <- log2(res$OR)
  OR[which(OR<=-ORmax)] <- -ORmax
  OR[which(OR>=ORmax)] <- -ORmax
  ...
  # Plot:
  XRANGE <- pretty(range(OR))
  XLIM <- c(min(XRANGE),max(XRANGE))
  YRANGE <- pretty(range(p))
  ...
  pbonf <- -log10(0.05/(nrow(p)*ncol(p)))
  points(XLIM,c(pbonf,pbonf),lty=2,lwd=1,type="l")
  title(main=main,xlab="log2(OR)",ylab="-10log(p)")
  COL <- rownames(p)
  Dummi <- sapply(1:nrow(p),function(x) points(OR[x,],p[x,],bg=COL[x],pch=21,lwd=2))
  # Legend:
  par(mar=c(0,0,0,0))
  plot.new()
  plot.window(xlim=XLIM,ylim=c(0,1))
  legend("center",legend=paste0("ME",COL),fill=COL,ncol=7,text.font=2,cex=0.7)
}
