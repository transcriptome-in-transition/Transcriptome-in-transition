...
plot.sample.similarity.vs.module.eg <- function(dat,net,Zcut=2.5,hold=TRUE){
	## Defining a Subfunction for each plot:
	plot.me_zk <- function(me,zk,col,Zcut=Zcut){
		## Settings Figure:
		...
		if(length(Ind0)>0){
			...
			points(x=me[Ind0],y=Z.k[Ind0],bg=rgb(1,0,0,0.5),pch=21)
		...
		if(length(Ind1)>0){
			points(x=me[Ind1],y=Z.k[Ind1],bg=rgb(0,0,1,0.5),pch=21)
		}
		## Do some Figure Pimping:
		...
		box()
		TEST <- cor.test(me,zk)
		title(main=paste0(col," COR=",as.character(signif(cor(me,zk),3))," p=",signif(TEST$p.value,3)),ylab="Z.k",xlab="Module Expression")	
		if(hold){
	...
	for(i in 1:ncol(ME)){
		plot.me_zk(ME[,i],Z.k,colnames(ME)[i],Zcut)
	}
}
